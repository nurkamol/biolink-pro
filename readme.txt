...
* New per-page slug editor in the Design page's new Page card. Edit a page's URL without touching its title.
...
= 2.3.0 =
* Shortcode support! `[biolink id="123"]` or `[biolink slug="alvasti"]` embeds a full bio page inside any post / page / widget. `[biolink_block id="123" uuid="abc"]` renders just one block (a single product card, donation form, etc.). Each embed is independently themed and coexists with the host page styles.

= 2.2.2 =
* Locked content now reveals inline — clicking a padlock opens a modal on the same page, and the correct passcode swaps in the rendered block (YouTube embed, image gallery, etc.) without a page reload or redirect to a separate unlock URL. The standalone unlock page stays as a no-JS fallback.

= 2.2.1 =
* Fix: the passcode gate in v2.2.0 only applied to Link blocks — setting a passcode on YouTube / Image Gallery / Donation / etc. still rendered the content publicly. Every block type is now gated at PageRenderer.
* Signed cookie persists unlock state for 30 days so visitors don't re-enter the passcode on every page load.
...
* New centralized Add modal replaces the inline block inserter.
* New card-based Design page replaces the tabbed settings rail.
* Per-block on/off toggle on every link row.
* Coming-soon stubs for Earn, Audience, Social planner, IG auto-reply, Link shortener, Post ideas — full feature work tracked in v2.1+.
